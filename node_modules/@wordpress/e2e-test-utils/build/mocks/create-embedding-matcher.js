...
function parameterEquals(parameterName, value) {
  return function (request) {
    return new URL(request.url()).searchParams.get(parameterName) === value;
  };
}
/**
...
function isRESTRoute(path) {
  return function (request) {
    return parameterEquals('rest_route', path)(request) || new URL(request.url()).pathname.endsWith((0, _path.join)('/wp-json', path));
  };
}
/**
...
function createEmbeddingMatcher(url) {
  return function (request) {
    return isRESTRoute('/oembed/1.0/proxy')(request) && parameterEquals('url', url)(request);
  };
}

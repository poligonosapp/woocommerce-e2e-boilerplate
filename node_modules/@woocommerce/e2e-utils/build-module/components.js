...
import { uiUnblocked, verifyCheckboxIsUnset, setCheckbox, unsetCheckbox, evalAndClick, backboneUnblocked, waitForSelectorWithoutThrow } from './page-utils';
...
const verifyAndPublish = async noticeText => {
  // Wait for auto save
  await waitForTimeout(2000); // Publish product
...
const completeOnboardingWizard = async () => {
  ...
  await merchant.runSetupWizard(); // Fill store's address - first line
  ...
  await verifyCheckboxIsUnset('.components-checkbox-control__input'); // Wait for "Continue" button to become active
...
const createVariableProduct = async function () {
  ...
  const {
    id
  } = await factories.products.variable.create(varProduct); // create the variable product
...
const createGroupedProduct = async function () {
  ...
  for (const simpleProduct of groupedProducts) {
    const {
      id
    } = await factories.products.simple.create(simpleProduct);
  ...
  const {
    id
  } = await factories.products.grouped.create(groupedProductRequest);
  return id;
};
/**
...
const createSimpleOrder = async function () {
  ...
  await merchant.openNewOrder(); // Make sure we're on the add order page
...
const addProductToOrder = async (orderId, productName) => {
  await merchant.goToOrder(orderId); // Add a product to the order
  ...
  await backboneUnblocked(); // Verify the product we added shows as a line item now
...
const addShippingZoneAndMethod = async function (zoneName) {
  ...
  await merchant.openNewShipping(); // Fill shipping zone name
  ...
  await uiUnblocked(); // Fill shipping zone postcode if needed otherwise just put empty space
  ...
  await uiUnblocked(); // Add shipping zone method
...
const deleteAllEmailLogs = async () => {
  await merchant.openEmailLog(); // Make sure we have emails to delete. If we don't, this selector will return null.
...
const deleteAllShippingZones = async () => {
  await merchant.openSettings('shipping'); // Delete existing shipping zones.

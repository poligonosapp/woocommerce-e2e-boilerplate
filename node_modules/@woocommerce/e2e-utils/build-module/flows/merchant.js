...
const merchant = {
  ...
  logout: async () => {
    ...
    const logoutLinks = await page.$$eval('#wp-admin-bar-logout a', am => am.filter(e => e.href).map(e => e.href));
  ...
  verifyOrder: async function (orderId, productName, productPrice, quantity, orderTotal) {
    let ensureCustomerRegistered = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
    await merchant.goToOrder(orderId); // Verify that the order page is indeed of the order that was placed
    // Verify order number

    await expect(page).toMatchElement('.woocommerce-order-data__heading', {
    ...
    await expect(page).toMatchElement('.woocommerce-Price-amount.amount', {
      text: productPrice
    ...
    if (ensureCustomerRegistered) {
      ...
      await expect(page).toMatchElement('label[for="customer_user"] a[href*=user-edit]', {
        text: 'Profile'
      });
    }
  },
  ...
  updateWordPress: async () => {
    ...
    if (null !== (await page.$('form[action="update-core.php?action=do-core-upgrade"][name="upgrade"]'))) {
      await Promise.all([expect(page).toClick('input.button-primary'), // The WordPress update can take some time, so setting a longer timeout here
  ...
  updatePlugins: async () => {
    ...
    if (null !== (await page.$('form[action="update-core.php?action=do-plugin-upgrade"][name="upgrade-plugins"]'))) {
      ...
      await Promise.all([expect(page).toClick('#upgrade-plugins'), page.waitForNavigation({
        waitUntil: 'networkidle0'
      })]);
    }
  },
  ...
  updateThemes: async () => {
    ...
    if (null !== (await page.$('form[action="update-core.php?action=do-theme-upgrade"][name="upgrade-themes"]'))) {
      ...
      await Promise.all([expect(page).toClick('#upgrade-themes'), page.waitForNavigation({
        waitUntil: 'networkidle0'
      })]);
    }
  },
  ...
  */
  uploadAndActivatePlugin: async (pluginFilePath, pluginName) => {
    await merchant.openPlugins(); // Deactivate and delete the plugin if it exists
    ...
    await uploader.uploadFile(pluginFilePath); // Manually update the button to `enabled` so we can submit the file
  ...
  */
  deactivatePlugin: async function (pluginName) {
    let deletePlugin = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    let pluginSlug = getSlug(pluginName);
  ...
  */
  deletePlugin: async pluginName => {
    let pluginSlug = getSlug(pluginName);
    await expect(page).toClick(`a#delete-${pluginSlug}`); // Wait for Ajax calls to finish
   ...
   */
  collapseAdminMenu: async function () {

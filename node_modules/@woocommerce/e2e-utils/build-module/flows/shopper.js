...
const shopper = {
  ...
  addToCartFromShopPage: async productIdOrTitle => {
    if (Number.isInteger(productIdOrTitle)) {
      ...
      const addToCartXPath = `//li[contains(@class, "type-product") and a/h2[contains(text(), "${productIdOrTitle}")]]` + '//a[contains(@class, "add_to_cart_button") and contains(@class, "ajax_add_to_cart")';
      const [addToCartButton] = await page.$x(addToCartXPath + ']');
      await addToCartButton.click(); // @todo: Update to waitForXPath when available in Puppeteer api.
  ...
  searchForProduct: async prouductName => {
    ...
    await expect(page).toClick('.wp-block-search__button'); // Single search results may go directly to product page

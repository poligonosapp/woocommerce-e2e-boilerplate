...
exports.verifyAndPublish = exports.deleteAllShippingZones = exports.deleteAllEmailLogs = exports.createVariableProduct = exports.createSimpleProductWithCategory = exports.createSimpleProduct = exports.createSimpleOrder = exports.createOrder = exports.createGroupedProduct = exports.createCoupon = exports.completeOnboardingWizard = exports.clickUpdateOrder = exports.batchCreateOrders = exports.addShippingZoneAndMethod = exports.addProductToOrder = void 0;
...
var _withRestApi = require("./flows/with-rest-api");
...
const verifyAndPublish = async noticeText => {
  // Wait for auto save
  await (0, _utils.waitForTimeout)(2000); // Publish product

  await expect(page).toClick('#publish');
  await page.waitForSelector('.updated.notice'); // Verify
...
const waitAndClickPrimary = async function () {
  let waitForNetworkIdle = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
...
const completeOnboardingWizard = async () => {
  ...
  await _flows.merchant.runSetupWizard(); // Fill store's address - first line

  await expect(page).toFill('#inspector-text-control-0', config.get('addresses.admin.store.addressfirstline')); // Fill store's address - second line

  await expect(page).toFill('#inspector-text-control-1', config.get('addresses.admin.store.addresssecondline')); // Fill country and state where the store is located

  await expect(page).toFill('.woocommerce-select-control__control-input', config.get('addresses.admin.store.countryandstate')); // Fill the city where the store is located

  await expect(page).toFill('#inspector-text-control-2', config.get('addresses.admin.store.city')); // Fill postcode of the store

  await expect(page).toFill('#inspector-text-control-3', config.get('addresses.admin.store.postcode')); // Verify that checkbox next to "I'm setting up a store for a client" is not selected

  await (0, _pageUtils.verifyCheckboxIsUnset)('.components-checkbox-control__input'); // Wait for "Continue" button to become active

  await page.waitForSelector('button.is-primary:not(:disabled)'); // Click on "Continue" button to move to the next step
  ...
  expect(industryCheckboxes).toHaveLength(8); // Select all industries including "Other"
  ...
  await expect(page).toFill('.components-text-control__input', config.get('onboardingwizard.industry')); // Wait for "Continue" button to become active

  await waitAndClickPrimary(); // Product types section
  // Query for the product types checkboxes

  const productTypesCheckboxes = await page.$$('.components-checkbox-control__input');
  expect(productTypesCheckboxes).toHaveLength(7); // Select Physical and Downloadable products
  ...
  await waitAndClickPrimary(); // Business Details section
  // Temporarily add delay to reduce test flakiness

  await page.waitFor(2000); // Query for the <SelectControl>s

  const selectControls = await page.$$('.woocommerce-select-control');
  expect(selectControls).toHaveLength(2); // Fill the number of products you plan to sell
  ...
  await waitAndClickPrimary(false); // Skip installing extensions

  await (0, _pageUtils.unsetCheckbox)('.components-checkbox-control__input');
  await (0, _pageUtils.verifyCheckboxIsUnset)('.components-checkbox-control__input');
  await waitAndClickPrimary(); // Theme section

  await waitAndClickPrimary(); // End of onboarding wizard

  if (_flows.IS_RETEST_MODE) {
    // Home screen modal can't be reset via the rest api.
    return;
  } // Wait for homescreen welcome modal to appear


  let welcomeHeader = await (0, _pageUtils.waitForSelectorWithoutThrow)('.woocommerce__welcome-modal__page-content');
...
exports.completeOnboardingWizard = completeOnboardingWizard;

const createSimpleProduct = async function () {
  let productTitle = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : simpleProductName;
  let productPrice = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : simpleProductPrice;
  let additionalProps = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  ...
  const product = await _factories.default.products.simple.create(newProduct);
  return product.id;
};
/**
...
exports.createSimpleProduct = createSimpleProduct;
...
const createSimpleProductWithCategory = async (productName, productPrice, categoryName) => {
  ...
  const categoryId = await _withRestApi.withRestApi.createProductCategory(categoryName);
  const product = await _factories.default.products.simple.create({
    name: productName,
    regularPrice: productPrice,
    categories: [{
      id: categoryId
...
exports.createSimpleProductWithCategory = createSimpleProductWithCategory;

const createVariableProduct = async function () {
  let varProduct = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : defaultVariableProduct;
  const {
    attributes
  } = varProduct;
  const {
    id
  } = await _factories.default.products.variable.create(varProduct); // create the variable product
  ...
  const createVariation = aIdx => {
    ...
    const isLastAttribute = aIdx === attributes.length - 1; // Add each attribute value to the buffer.
  ...
  createVariation(aIdx); // Set some properties of 1st variation

  variations[0].regularPrice = '9.99';
  variations[0].virtual = true; // Set some properties of 2nd variation

  variations[1].regularPrice = '11.99';
  variations[1].virtual = true; // Set some properties of 3rd variation
  ...
  variations[2].manage_stock = true; // Use API to create each variation

  for (const v of variations) {
    await _factories.default.products.variation.create({
      productId: id,
      variation: v
    });
  }
...
exports.createVariableProduct = createVariableProduct;

const createGroupedProduct = async function () {
  let groupedProduct = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : defaultGroupedProduct;
  const {
    name,
    groupedProducts
  } = groupedProduct;
  const simpleProductIds = [];
  let groupedProductRequest; // Using the api, create simple products to be grouped

  for (const simpleProduct of groupedProducts) {
    const {
      id
    } = await _factories.default.products.simple.create(simpleProduct);
  ...
  const {
    id
  } = await _factories.default.products.grouped.create(groupedProductRequest);
  return id;
};
/**
...
exports.createGroupedProduct = createGroupedProduct;

const createOrder = async function () {
  let orderOptions = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

  const newOrder = _objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread({}, orderOptions.status && {
...
const createSimpleOrder = async function () {
  let orderStatus = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'Pending payment';
  // Go to 'Add new order' page
  await _flows.merchant.openNewOrder(); // Make sure we're on the add order page

  await expect(page.title()).resolves.toMatch('Add new order'); // Set order status

  await expect(page).toSelect('#order_status', orderStatus); // Wait for auto save

  await page.waitFor(2000); // Create the order
...
const batchCreateOrders = async statuses => {
  ...
  const path = '/wc/v3/orders/batch'; // Create an order per status
...
exports.batchCreateOrders = batchCreateOrders;

const addProductToOrder = async (orderId, productName) => {
  await _flows.merchant.goToOrder(orderId); // Add a product to the order
  ...
  await (0, _pageUtils.backboneUnblocked)(); // Verify the product we added shows as a line item now
...
const createCoupon = async function () {
  let couponAmount = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '5';
  let discountType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'Fixed cart discount';
  let couponType;
...
const addShippingZoneAndMethod = async function (zoneName) {
  ...
  await _flows.merchant.openNewShipping(); // Fill shipping zone name

  await page.waitForSelector('input#zone_name');
  await expect(page).toFill('input#zone_name', zoneName); // Select shipping zone location

  await expect(page).toSelect('select[name="zone_locations"]', zoneLocation);
  await (0, _pageUtils.uiUnblocked)(); // Fill shipping zone postcode if needed otherwise just put empty space
  ...
  await (0, _pageUtils.uiUnblocked)(); // Add shipping zone method
  ...
  await (0, _pageUtils.uiUnblocked)();
};
/**
...
exports.addShippingZoneAndMethod = addShippingZoneAndMethod;

const clickUpdateOrder = async function (noticeText) {
  let waitForSave = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
exports.clickUpdateOrder = clickUpdateOrder;

const deleteAllEmailLogs = async () => {
  await _flows.merchant.openEmailLog(); // Make sure we have emails to delete. If we don't, this selector will return null.

  if ((await page.$('#bulk-action-selector-top')) !== null) {
    await (0, _pageUtils.setCheckbox)('#cb-select-all-1');
...
exports.deleteAllEmailLogs = deleteAllEmailLogs;

const deleteAllShippingZones = async () => {
  await _flows.merchant.openSettings('shipping'); // Delete existing shipping zones.
  ...
  try {
    ...
    if (zone) {
      ...
      await page.evaluate(() => {
        document.querySelector('.wc-shipping-zone-name .row-actions').style.left = '0';
      });

      while (zone) {
        await (0, _pageUtils.evalAndClick)('.wc-shipping-zone-delete');
        await (0, _pageUtils.uiUnblocked)();
    ...
    ;
  } catch (error) {// Prevent an error here causing the test to fail.
  }

  ;
};

exports.deleteAllShippingZones = deleteAllShippingZones;
